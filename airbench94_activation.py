...
#
# Modified for ESE 3060 Final Project: Added configurable activation function parameter
# Hypothesis: Replacing GELU with ReLU² will speed up training while maintaining accuracy
...
hyp = {
    'opt': {
        ...
        'warmup_ratio': 0.23,       # fraction of training steps used for LR warmup
    },
    'aug': {
        'flip': True,
        'translate': 2,
    ...
    'net': {
        ...
        'activation': 'gelu',   # EXPERIMENT PARAMETER: 'gelu', 'relu', 'relu_squared', 'swish'
    },
}
...
def get_activation(name):
    ...
    if name == 'gelu':
        ...
        return nn.SiLU()  # SiLU is PyTorch's name for Swish
    else:
        raise ValueError(f"Unknown activation: {name}. Choose from: gelu, relu, relu_squared, swish")
...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        ...
        self.activ = get_activation(hyp['net']['activation'])
...
def make_net():
    ...
    net = nn.Sequential(
        ...
        get_activation(hyp['net']['activation']),  # Use configurable activation
...
def main(run):
    ...
    def get_lr(step):
        warmup_ratio = hyp['opt']['warmup_ratio']
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='CIFAR-10 Airbench Training with Activation Function Experiment')
    ...
    # Set the activation function from command line
    hyp['net']['activation'] = args.activation
    
    with open(sys.argv[0]) as f:
        code = f.read()

    print(f"\n{'='*60}")
    print(f"EXPERIMENT: activation = {args.activation}")
    print(f"{'='*60}\n")
    ...
    # Save log with activation in filename for easy identification
    log = {
        'code': code, 
        'accs': accs,
        'times': times,
    ...
    log_dir = os.path.join('logs', f'activation_{args.activation}')
